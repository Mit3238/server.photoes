document.addEventListener('DOMContentLoaded', () => {
    ...
    const stopProcessingButton = document.getElementById('stop-processing-button');
    ...
    async function fetchPhotos(page = 1) {
        ...
        try {
            const response = await fetch(`/api/photos/list?page=${page}&per_page=${homePerPage}`);
            if (!response.ok) {
        ...
        fetch('/api/persons/list')
            ...
            .then(data => {
                ...
                data.persons.forEach(person => {
                    ...
                    const img = document.createElement('img');
                    img.src = person.facefile ? `/api/photos/uploads/faces/${person.facefile}` : 'img/placeholder_person.png';
    ...
    if (uploadForm && photoInput && uploadButton && uploadStatusMessage) {
        uploadForm.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                if (response.ok) {
                    uploadStatusMessage.textContent = `Successfully uploaded ${result.uploaded?.length || 0} photo(s).`;
                    ...
                    uploadStatusMessage.textContent = `Upload failed: ${result.error || 'Unknown error'}`;
                    uploadStatusMessage.className = 'status-message error';
                }
            } catch (error) {
                ...
                setTimeout(() => { /* ... (timeout logic unchanged) ... */
                    if (uploadStatusMessage.textContent.startsWith('Successfully uploaded') || uploadStatusMessage.textContent.startsWith('Upload failed')) {
    ...
    } else { console.error('Upload form elements not found!'); }

    // --- Processing Controls Logic (Unchanged) ---
    if (startProcessingButton && stopProcessingButton && processingStatusMessage) {
        startProcessingButton.addEventListener('click', async () => { /* ... (unchanged) ... */
            ...
            try {
                ...
                if (response.ok) {
                    processingStatusMessage.textContent = result.message || 'Processing started successfully.';
                    processingStatusMessage.className = 'status-message success';
                } else {
                    processingStatusMessage.textContent = `Error: ${result.message || 'Unknown error'}`;
        ...
        stopProcessingButton.addEventListener('click', async () => { /* ... (unchanged) ... */
            ...
            try {
                ...
                if (response.ok) {
                    processingStatusMessage.textContent = result.message || 'Processing stopped successfully.';
                    processingStatusMessage.className = 'status-message success';
                } else {
                    processingStatusMessage.textContent = `Error: ${result.message || 'Unknown error'}`;
                    processingStatusMessage.className = 'status-message error';
    ...
    } else { console.error('Processing control elements not found!'); }
